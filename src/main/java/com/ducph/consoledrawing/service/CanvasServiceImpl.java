...
import com.ducph.consoledrawing.model.Point;
...
public class CanvasServiceImpl implements CanvasService {
    ...
    public CanvasServiceImpl(int w, int h) {
        width = w;
        height = h;

        cachedCanvasArray = new char[this.height][this.width];
        
        // Replace all item value with space
    ...
    private void drawLine(int x1, int y1, int x2, int y2) {
        ...
        for (int row = y1 - 1; row <= y2 - 1 && row < height; row++) {
            ...
            for (int col = x1 - 1; col <= x2 - 1 && col < width; col++) {
                cachedCanvasArray[row][col] = LINE_CHAR;
            }
        }
    }
    ...
    private void fillBucket(int x, int y, char color) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (cachedCanvasArray[pop.getX()][pop.getY()] == originalChar) {
                cachedCanvasArray[pop.getX()][pop.getY()] = color;
            }
